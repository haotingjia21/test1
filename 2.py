...
'''
the modified lines are from 39 - 56 in function all_ofrs()
'''
...
def all_orfs(codon):
    ...
    while x == 1:
        ...
        # ATG, AGT, TTG, GTG, or CTG
        if 'ATG' in remaining:
            start1 = remaining.index('ATG')
        if 'AGT' in remaining:
            start2 = remaining.index('AGT')
        if 'TTG' in remaining:
            start3 = remaining.index('TTG')
        if 'GTG' in remaining:
            start4 = remaining.index('GTG')
        if 'CTG' in remaining:
            start5 = remaining.index('CTG')
